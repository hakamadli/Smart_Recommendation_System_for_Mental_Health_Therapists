...
class _SettingsPageState extends State<SettingsPage> {
  ...
  final Stream<QuerySnapshot> _therapistStream =
    ...
    await therapistsRef
        ...
        .then((snapshot) => {
              snapshot.docs.forEach((document) {
                ...
                _experienceController.text = data['exp'].toString();
                _orgController.text = data['organization'];
  ...
  Future<void> _deleteUser(String therapistID) async {
    await FirebaseFirestore.instance
  ...
  Future<void> _updateUserName(String newName) async {
    await therapistsRef.doc(user.uid).update({'name': newName});
  }

  Future<void> _updateUserExp(String newExp) async {
    await therapistsRef.doc(user.uid).update({'exp': int.parse(newExp)});
  }

  Future<void> _updateUserOrg(String newOrg) async {
    await therapistsRef.doc(user.uid).update({'organization': newOrg});
  }
  ...
  Future<void> _updateUserEmail(
    ...
    // );
    await user.reload();
    therapistsRef.doc(user.uid).update({'email': newEmail}).then(
                                ...
                                builder: (BuildContext context) {
                                  return AlertDialog(
                                    ...
                                    content: Column(
                                      ...
                                      children: const [
                                        Text(
                                            "This is a mobile application that uses recommendation & machine learning techniques to diagnose mental illness with DASS-21 scoring, provide selfcare treatments, manage & improve youth mental wellbeing\n"),
