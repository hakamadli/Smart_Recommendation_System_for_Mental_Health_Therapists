...
class EditHelpPage extends StatefulWidget {
  const EditHelpPage({Key? key, required this.resource}) : super(key: key);
  final DocumentSnapshot resource;

  @override
  State<EditHelpPage> createState() => _EditHelpPageState(resource: resource);
}

class _EditHelpPageState extends State<EditHelpPage> {
  _EditHelpPageState({Key? key, required this.resource});
  final DocumentSnapshot resource;
  ...
  // set details
  Future<void> setHelpDetails() async {
    var helpSnapshot = await FirebaseFirestore.instance
        .collection('helpline_and_resources')
        .doc(resource.id)
        .get();
    setState(() {
      _nameController.text = helpSnapshot.get('helpName');
      _hotlineController.text = helpSnapshot.get('hotline');
      _emailController.text = helpSnapshot.get('helpEmail');
      _websiteController.text = helpSnapshot.get('website');
      _descController.text = helpSnapshot.get('description');
    });
  }
  ...
  Future addResourceData(String resourceID, String uid, String name, String hotline, String email,
